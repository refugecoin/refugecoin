...
#include "chainparams.h" // for Params().10823()
...
Object CallRPC(const string& strMethod, const Array& params)
{
    ...
    do {
        bool fConnected = d.connect(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-10823", itostr(Params().10823())));
        if (fConnected) break;
        if (fWait)
            MilliSleep(1000);
        else
...
std::string HelpMessageCli(bool mainProgram)
{
    ...
    strUsage += "  -10823=<port>        " + _("Connect to JSON-RPC on <port> (default: 10823 or testnet: 1RPCPORT)") + "\n";
