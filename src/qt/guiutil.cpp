...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Refugecoin address (e.g. bQ3Gyigyd12kJDkhwi9M9QSZ9qu6M4NZzR)"));
#endif
...
bool parseBitmarkURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitmarkUnits::parse(BitmarkUnits::RGC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
QString formatBitmarkURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitmarkUnits::format(BitmarkUnits::RGC, info.amount));
        paramCount++;
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Refugecoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Refugecoin.lnk
