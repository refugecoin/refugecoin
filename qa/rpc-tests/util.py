...
Refugecoind_processes = []

def initialize_chain(test_dir):
    """
    Create (or copy from cache) a 200-block-long chain and
    4 wallets.
    Refugecoind and Refugecoin-cli must be in search path.
    """

    if not os.path.isdir(os.path.join("cache", "node0")):
        devnull = open("/dev/null", "w+")
        # Create cache directories, run Refugecoinds:
        for i in range(4):
            ...
            with open(os.path.join(datadir, "bitmark.conf"), 'w') as f:
                ...
                f.write("10823="+str(START_RPC_PORT+i)+"\n");
            args = [ "Refugecoind", "-keypool=1", "-datadir="+datadir ]
            if i > 0:
                args.append("-connect=127.0.0.1:"+str(START_P2P_PORT))
            Refugecoind_processes.append(subprocess.Popen(args))
            subprocess.check_call([ "Refugecoin-cli", "-datadir="+datadir,
...
def start_nodes(num_nodes, dir):
    # Start Refugecoinds, and wait for RPC interface to be up and running:
    devnull = open("/dev/null", "w+")
    for i in range(num_nodes):
        datadir = os.path.join(dir, "node"+str(i))
        args = [ "Refugecoind", "-datadir="+datadir ]
        Refugecoind_processes.append(subprocess.Popen(args))
        subprocess.check_call([ "Refugecoin-cli", "-datadir="+datadir,
                                  "-rpcwait", "getblockcount"], stdout=devnull)
    devnull.close()
...
def wait_Refugecoinds():
    # Wait for all Refugecoinds to cleanly exit
    for Refugecoind in Refugecoind_processes:
        Refugecoind.wait()
    del Refugecoind_processes[:]
